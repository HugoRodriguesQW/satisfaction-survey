...
export function BuilderPanel() {
    ...
    function createQuestion(type: QuestionTypes): Question {
        switch (type) {
            case "text-input":
                return merge(clone(TextInputQuestion), { type });
            case "selection":
                return merge(clone(SelectionQuestion), { type });
            case "text-message":
                return merge(clone(TextMessageQuestion), { type });
            case "slider":
                return merge(clone(SliderQuestion), { type });
        }
    }
                                        ...
                                        <div className="flex max-w-[600px] flex-wrap gap-3 items-center"> {
                                            question.selection.options.map((option, j) => {
                                                ...
                                                return <button
                                                    ...
                                                    className={
                                                        twMerge("bg-gradient-to-br to-violet-500/80 from-neon-mid/70 p-2 px-6 rounded-md min-h- flex-[1_auto]")
                                                    } >
                                                    {option.text}
                                                </button>
                                            })
